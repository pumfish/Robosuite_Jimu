...
class Jimu(SingleArmEnv):
    ...
    def __init__(
        ...
        if jimu_src_matrix == None:
            ...
            def find_jimu_pos(m):
                z = np.random.randint(0, m.shape[0])
            ...
            #self.jimu_src_m[0,0,0] = True
            #self.jimu_src_m[0,1,0] = True
            #self.jimu_src_m[0,2,0] = False
        ...
        print("jimu shape", self.jimu_shape)
        print("src num", self.jimu_src_m.sum(), "tgt num", self.jimu_tgt_m.sum())
        print("src matrix")
        print(self.jimu_src_m)
        print("tgt matrix")
        print(self.jimu_tgt_m)
    ...
    def _load_model(self):
        ...
        self.jimu_src_cubes = []
        self.jimu_tgt_cubes = []
        for z in range(self.jimu_src_m.shape[0]):
            for y in range(self.jimu_src_m.shape[1]):
                for x in range(self.jimu_src_m.shape[2]):
                    if self.jimu_src_m[z,y,x]:
                        ...
                        #pad_y_range = [0.04*y+0.02, 0.04*(y+1)+0.02]
                        #y_range = [pad_y_range[0]+0.005, pad_y_range[1]-0.005]
                        #pad_x_range = [0.04*x-0.02*max_size, 0.04*(x+1)-0.02*max_size]
                        #x_range = [pad_x_range[0]+0.005, pad_x_range[1]-0.005]


                        print("src cube", obj_idx_name, "pos", x_range, y_range)
            ...
            for y in range(self.jimu_tgt_m.shape[1]):
                for x in range(self.jimu_tgt_m.shape[2]):
                    if not self.jimu_src_m[z,y,x] and self.jimu_tgt_m[z,y,x]:
                        ...
                        self.placement_initializer.append_sampler(UniformFixSampler(
                                name="TgtObjectSampler_"+obj_idx_name,
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=self.jimu_src_cubes + self.jimu_tgt_cubes,
        )
