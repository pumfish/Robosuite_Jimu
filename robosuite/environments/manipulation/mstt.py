...
from robosuite.models.arenas import PlayArena
...
from robosuite.utils.placement_samplers import SequentialCompositeSampler, UniformRandomSampler,UniformFixSampler
...
class Mstt(SingleArmEnv):
    ...
    def load_lift_task(self, index, central, table_offset):
        tex_attrib = {
            "type": "cube",
        }
        mat_attrib = {
        ...
        placement_initializer = UniformRandomSampler(
            name=str(index)+"ObjectSampler",
            mujoco_objects=cube,
            x_range=[central[0]-0.03, central[0]+0.03],
            y_range=[central[1]-0.03, central[1]+0.03],
            rotation=None,
            ensure_object_boundary_in_range=False,
            ensure_valid_placement=True,
        ...
        return [cube], [placement_initializer]

    def load_stack_task(self, index, central, table_offset, pre_pos=None):
        print(index, "offset", table_offset)
        tex_attrib = {
            "type": "cube",
        }
        mat_attrib = {
        ...
        if pre_pos == None:
            ...
            cubeB = XBoxVisualObject(str(index)+"visual_cubeB")
        cubes = [cubeA, cubeB]
        # Create placement initializer
        if pre_pos == None:
            placement_initializer = UniformRandomSampler(
                ...
                x_range=[central[0]-0.08, central[0]+0.08],
                y_range=[central[1]-0.08, central[1]+0.08],
                rotation=None,
                ensure_object_boundary_in_range=False,
                ensure_valid_placement=True,
                reference_pos=table_offset,
            ...
            self.objects += cubes
            return cubes, [placement_initializer]
        else:
            placement_initializerA = UniformFixSampler(
                name=str(index)+"ObjectSamplerA",
                mujoco_objects=cubeA,
                x_range=[central[0]-0.08, central[0]+0.08],
                y_range=[central[1]-0.08, central[1]+0.08],
                rotation=None,
                ensure_object_boundary_in_range=False,
            ...
            placement_initializerB = UniformRandomSampler(
                name=str(index)+"ObjectSamplerVB",
                mujoco_objects=cubeB,
                x_range=[pre_pos[0]-0.03, pre_pos[0]+0.03],
                y_range=[pre_pos[1]-0.03, pre_pos[1]+0.03],
                rotation=None,
                ensure_object_boundary_in_range=False,
                ensure_valid_placement=True,
                reference_pos=table_offset,
            ...
            self.objects.append(cubeA)
            self.visual_objects.append(cubeB)
            return cubes, [placement_initializerA, placement_initializerB]

    

    def load_nut_assembly_task(self, index, central, table_offset, pre_pos=None):
        # define nuts
        nuts = []
        all_nut_names = ["SquareNut", "RoundNut"]
        all_nut_names_visual = ["SquareVisualNut", "RoundVisualNut"]
        if pre_pos == None:
            #nut_names = [all_nut_names[np.random.randint(0,2)]]
            nut_names = [all_nut_names[index%2]]
        else:
            nut_names = [all_nut_names_visual[index%2]]
        #nut_names = ["SquareVisualNut"]
        ...
        #for nut_name, default_y_range in zip(nut_names, ([0.01, 0.125], [-0.125, -0.01])):
        for nut_name, default_y_range in zip(nut_names, ([[-0.125, 0.125]])):
            if nut_name == "SquareNut":
                nut_obj = SquareNutObject(name = str(index)+nut_name)
                self.objects.append(nut_obj)
                nuts.append(nut_obj)
            elif nut_name == "RoundNut":
                nut_obj = RoundNutObject(name = str(index)+nut_name)
                self.objects.append(nut_obj)
                nuts.append(nut_obj)
            elif nut_name == "SquareVisualNut":
                nut_obj = SquareNutVisualObject(name = str(index)+nut_name)
                self.visual_objects.append(nut_obj)
                nuts.append(nut_obj)
            elif nut_name == "RoundVisualNut":
                nut_obj = RoundNutVisualObject(name = str(index)+nut_name)
                self.visual_objects.append(nut_obj)
                nuts.append(nut_obj)
            else:
                raise NotImplementedError
            if pre_pos == None:
                placement_initializers.append(
                    UniformRandomSampler(
                        name=str(index)+f"{nut_name}Sampler",
                        mujoco_objects=nut_obj,
                        x_range=[central[0]-0.005, central[0]],
                        y_range=[central[1]+default_y_range[0], central[1]+default_y_range[1]],
                        rotation=None,
                        rotation_axis="z",
                        ensure_object_boundary_in_range=False,
                        ensure_valid_placement=True,
                ...
                placement_initializers.append(
                    UniformFixSampler(
                        name=str(index)+f"{nut_name}Sampler",
                        mujoco_objects=nut_obj,
                        x_range=[pre_pos[0]-0.005, pre_pos[0]],
                        y_range=[pre_pos[1]+default_y_range[0], pre_pos[1]+default_y_range[1]],
                        rotation=None,
                        rotation_axis="z",
                        ensure_object_boundary_in_range=False,
                        ensure_valid_placement=False,
        ...
        return nuts, placement_initializers 
    ...
    def _load_model(self):
        ...
        small_centrals = [[-0.15, -0.3], [-0.15, 0.3], [0.15, -0.3], [0.15, 0.3]] 
        small_pre_poses = [[-0.25, -0.05], [-0.25, 0.45], [0.05, -0.05], [0.05, 0.45]]
        big_centrals = [[-0.15, -0.375], [-0.15, 0.125], [0.15, -0.375], [0.15, 0.125]] 
        big_pre_poses = [[-0.15, -0.125], [-0.15, 0.375], [0.15, -0.125], [0.15, 0.375]]

        table_offset = self.bin1_pos
        ...
        centrals_set = [
            ...
            [[-0.15, -0.375], [-0.15, 0.125], [0.15, -0.375], [0.15, 0.125]],
            [[-0.15, -0.375], [-0.15, 0.125], [0.15, -0.3], [0.15, 0.3]]
        ]
        pre_pos_set = [
            [None, [-0.25, -0.05], [-0.25, 0.45], [0.05, -0.05]],
            [None, [-0.15, -0.125], [-0.15, 0.375], [0.15, -0.125]],
            [None, [-0.15, -0.125], [-0.15, 0.375], [0.05, -0.05]]
        ]

        tasks = task_set[task_id]#[2,2,2,2]
        pre_poses = pre_pos_set[task_id]
        centrals = centrals_set[task_id]
        all_objs = []
        for i, task_type in enumerate(tasks):
            if task_type == 0:
                objs, placement_initializers = self.load_lift_task(i, small_centrals[i], table_offset)
            elif task_type == 1:
                #pre_pos = None if i==0 else small_pre_poses[i-1]
                objs, placement_initializers = self.load_stack_task(i, small_centrals[i], table_offset, pre_pos=pre_poses[i])
            elif task_type == 2:
                #pre_pos = None if i==0 else big_pre_poses[i-1]
                objs, placement_initializers = self.load_nut_assembly_task(i, big_centrals[i], table_offset, pre_pos=pre_poses[i])
            else:
                raise NotImplementedError
            all_objs += objs
            for p in placement_initializers:
                self.placement_initializer.append_sampler(p)
        print("placements", self.placement_initializer.samplers)
        for k in self.placement_initializer.samplers.keys():
            p = self.placement_initializer.samplers[k]
            print(k, p.reference_pos, p.z_offset, p.x_range, p.y_range)
        print("objects num", len(all_objs))#, "pi num", len(len(self.placement_initializer.samplers.values())))
        ...
        self.model = ManipulationTask(
            ...
            mujoco_robots=[robot.robot_model for robot in self.robots],
            #mujoco_objects=self.visual_objects + self.objects,
            mujoco_objects=all_objs#self.visual_objects + self.objects,
            #mujoco_objects=[]#all_objs#self.visual_objects + self.objects,
        )
